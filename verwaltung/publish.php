...
$Bearbeitungsstatus = $data['Bearbeitungsstatus']!='' ? "'".$data['Bearbeitungsstatus']."'" : "NULL";
$Bild = $data['Bild']!='' ? "'".$data['Bild']."'" : "NULL";
$Service_Note = $data['service_note']!='' ? "'".$data['service_note']."'" : "NULL";
$query_text = "UPDATE stellen SET lat=$lat, lng=$lng, Titel=$Titel, Problem=$Problem, Kategorie=$Kategorie, Bearbeitungsstatus=$Bearbeitungsstatus, Bild=$Bild, published=1 WHERE id=$id;";
//echo $query_text;
